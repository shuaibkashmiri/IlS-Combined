...
import { BsArrowRight, BsArrowLeft, BsClock, BsStars, BsPlayCircle, BsBookmark, BsBookmarkFill } from "react-icons/bs";
            ...
            return (
              <div
                ...
                style={{
                  ...
                  opacity,
                  transition: 'transform 1s cubic-bezier(.77,0,.18,1), opacity 0.3s ease',
                }}
              >
                <div className="w-full h-full relative group">
                  ...
                  <div className="relative p-6 h-full flex flex-col">
                    ...
                    <div className="flex justify-between items-start mb-4">
                      ...
                      >
                        {bookmarked[course.id] ? <BsBookmarkFill /> : <BsBookmark />}
                      </button>
                    </div>
                    ...
                    <div className="flex-1">
                      ...
                      <p className={`text-sm mb-4 line-clamp-3 ${isActive ? 'text-gray-300' : 'text-gray-400'}`}>
                        {course.description}
                      </p>
                    </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div className={`flex flex-col items-center justify-center p-2 rounded-xl
                          ${isActive 
                            ...
                            : 'bg-gray-700/50'}`}>
                          <BsClock className={`text-lg mb-1 ${isActive ? 'text-emerald-400' : 'text-gray-400'}`} />
                          <span className={`text-xs ${isActive ? 'text-emerald-400' : 'text-gray-400'}`}>
                            {course.duration}
                          </span>
                        </div>
                        <div className={`flex flex-col items-center justify-center p-2 rounded-xl
                          ${isActive 
                            ? 'bg-emerald-500/10 border border-emerald-500/20' 
                            : 'bg-gray-700/50'}`}>
                          <BsStars className={`text-lg mb-1 ${isActive ? 'text-emerald-400' : 'text-gray-400'}`} />
                          <span className={`text-xs ${isActive ? 'text-emerald-400' : 'text-gray-400'}`}>
                            {course.level}
                          </span>
                        </div>
