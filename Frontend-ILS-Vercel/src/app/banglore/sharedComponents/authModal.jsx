...
const AuthModal = ({
  ...
  const [pendingUser, setPendingUser] = useState({ email: "", password: "" });
  ...
  const handleOtpVerify = async () => {
    try {
      const result = await dispatch(
        verifyUserOtp({ email: pendingUser.email, otp })
      ).unwrap();
      await dispatch(getUserDetails());
      toast.success(result.message || "Account verified! You can now log in.");
      toggleModal();
      setFormData({ fullname: "", email: "", password: "" });
      setOtp("");
    } catch (error) {
  ...
  const handleResendOtp = async () => {
    try {
      const result = await dispatch(
        resendUserVerification(pendingUser.email)
      ).unwrap();
          ...
          {step === "otp" && (
            <div>
              ...
              <p className="mb-4 text-gray-600">
                Please enter the 6-digit OTP sent to <b>{pendingUser.email}</b>
              </p>
              <input
                type="text"
                value={otp}
              ...
              <button
                ...
                className="w-full bg-[#00965f] text-white py-3 rounded-lg font-medium hover:bg-[#164758] transition-colors duration-300 mb-2"
              >
                Verify OTP
              </button>
              <button
                onClick={handleResendOtp}
                className="w-full text-[#00965f] font-medium hover:text-[#164758] transition-colors mb-2"
              >
                Resend OTP
              </button>
              <button
